...
def read_epu_files(data_dir, countries_slugs):
    ...
    for country in countries_slugs:
        ...
        epu["epu_news_count"] = epu[epu_news_count_cols].sum(axis=1)
        epu["country"] = country
        epu = epu[
            [
                "date",
...
def read_epu_topics_files(data_dir, topics, countries_slugs):
    ...
    for country in countries_slugs:
        ...
        for topic in topics:
            ...
            epu_news_count_cols = [
                col for col in epu_topics.columns if "epu_count" in col
            ]
            epu_topics[f"{topic}_epu_news_count"] = epu_topics[epu_news_count_cols].sum(
                axis=1
            )
            epu_topics = epu_topics[
                ["date", "country", f"epu_{topic}", f"{topic}_epu_news_count"]
            ]
...
def read_sentiment_files(data_dir, countries_slugs):
    ...
    for country in countries_slugs:
        ...
        sentiment = sentiment.rename(
            columns={"score": "sentiment_score", "z_score": "sentiment_z_score"}
        )
...
def group_monthly(data):
    ...
    mean_columns = [
        col for col in data.columns if ("epu_" in col) | ("sentiment_" in col)
    ]
