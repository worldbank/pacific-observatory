...
VU_DAILY_PAGE_URLS = [
    f"https://www.dailypost.vu/search/?f=html&s=start_time&sd=desc&l=100&t=article&nsa=eedition&app%5B0%5D=editorial&o={num}&category=news"
...
urls_raw = dp.scrape_urls(
    VU_DAILY_PAGE_URLS, ["card-headline", "card-meta"], speed_up=True
)
...
urls_df["date"] = pd.to_datetime(
    urls_df["date"].apply(handle_mixed_dates), format="mixed"
)
...
urls_df = urls_df[
    (urls_df.url.str.startswith("https://www.dailypost.vu/news/"))
    & (urls_df.date > latest_date)
...
with tqdm(total=len(news_urls)) as pbar:
    for idx, url in enumerate(news_urls):
        ...
        if elements is not None:
            ...
            text = "Missing"
        dp_news.append(
            [url, urls_df["title"][idx], urls_df["date"][idx], text]
        )
...
previous_news_df = pd.read_csv(news_path).drop("Unnamed: 0", axis=1)
current_news_df = pd.DataFrame(
    dp_news, columns=["url", "title", "date", "news"]
)
...
news_df = (
    news_df[news_df["news_length"] >= 50]
    .reset_index(drop=True)
...
news_df = (
    news_df[
        (~news_df.title.str.startswith("Re:"))
        & (
            ~news_df.url.str.startswith(
                "https://www.dailypost.vu/news/letters/"
            )
        )
    ]
