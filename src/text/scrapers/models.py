...
class ArticleRecord(BaseModel):
    ...
    tags: Optional[List[str]] = Field(
        default_factory=list, description="Article tags/categories"
    )
...
class ThumbnailSelectorConfig(BaseModel):
    ...
    container: SelectorType = Field(
        ..., description="Selector(s) for thumbnail container elements"
    )
    ...
    date: Optional[SelectorType] = Field(
        default=None, description="Optional selector(s) for thumbnail dates"
    )
...
class ArticleSelectorConfig(BaseModel):
    ...
    tags: Optional[SelectorType] = Field(
        default=None, description="Optional selector(s) for article tags"
    )
...
class NewspaperConfig(BaseModel):
    ...
    selectors: SelectorsConfig = Field(
        ..., description="Structured selectors for data extraction"
    )
    ...
    cleaning: Optional[dict] = Field(
        default=None, description="Data cleaning configuration"
    )
    ...
    concurrency: Optional[int] = Field(
        default=10, description="Maximum concurrent requests for HTTP client"
    )
    rate_limit: Optional[float] = Field(
        default=0.1, description="Minimum delay between requests in seconds"
    )
    ...
    max_pages: Optional[int] = Field(
        default=None, description="Maximum number of listing pages to scrape"
    )
    max_articles: Optional[int] = Field(
        default=None, description="Maximum number of articles to scrape"
    )
    ...
    def listing_must_have_type(cls, v: dict) -> dict:
        ...
        if v["type"] not in ["pagination", "archive", "api"]:
            raise ValueError(
                "Listing type must be one of: pagination, archive, category, search"
            )
        return v
